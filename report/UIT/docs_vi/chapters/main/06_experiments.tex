...
\noindent \autoref{fig:news_pipeline} tóm tắt pipeline thu thập tin tức: Airflow điều phối theo lịch; scraper workers tải và trích xuất nội dung; hệ thống khử trùng lặp theo URL để crawl tăng dần; dữ liệu được lưu trữ để phục vụ các bước downstream.
...
\autoref{fig:benchmark} so sánh độ ổn định (khoảng cách giữa cặp dương/âm) của 5 mô hình tiên tiến nhất. Chúng tôi định nghĩa Khoảng cách Ổn định theo toán học như:
\begin{equation}
...
Kết quả cho thấy \texttt{dangvantuan/vietnamese-document-embedding} đạt độ ổn định tốt nhất (Gap Trung bình 0,265). \autoref{tab:embedding_benchmark} chi tiết hiệu suất trên các thách thức ngôn ngữ cụ thể, được định nghĩa như sau:
