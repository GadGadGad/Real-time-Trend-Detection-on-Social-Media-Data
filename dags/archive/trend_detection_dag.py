...
if AIRFLOW_AVAILABLE:
    with DAG(
        ...
        # Task 0: Crawling (Added for pipeline completeness, disabled by default for demo)
        RUN_LIVE_CRAWL = Variable.get("RUN_LIVE_CRAWL", "False").lower() == "true"

        task_crawl_social = BashOperator(
            task_id='crawl_social_media',
            bash_command=f'python "{PROJECT_ROOT}/run_crawlers.py" --all --category thoi-su kinh-doanh --pages 1' if RUN_LIVE_CRAWL else 'echo "Skipping live social crawl (Demo Mode)..."',
        )

        task_crawl_trends = BashOperator(
            task_id='crawl_google_trends',
            bash_command='echo "Skipping Google Trends crawl (using pre-computed snapshots)..."',
        )
        ...
        # Define task order
        [task_crawl_social, task_crawl_trends] >> task_load >> task_pipeline >> task_report >> task_cleanup
