...
\begin{frame}{Kiến trúc Mô hình đề xuất (Proposed Architecture)}
    ...
    \begin{block}{Ngăn xếp Công nghệ lõi}
        \begin{itemize}
            ...
            \item \textbf{Analysis:} PhoBERT (Sentiment) + Statistical Scoring.
            \item \textbf{GenAI:} Google Gemini (Zero-shot).
        \end{itemize}
    \end{block}
\end{frame}
...
\begin{block}{Công thức tính điểm Hot (G-Score)}
    ...
    \begin{itemize}
        ...
        \item $F_{score}$ (Social Interaction): Tổng lượng like, share, comment (Log-scale để giảm ảnh hưởng của outlier).
        \item $G_{score}$ (Google Search): Chỉ số quan tâm tìm kiếm (0-100).
        \item $\alpha, \beta, \gamma$: Các trọng số thực nghiệm (Ví dụ: 0.3, 0.5, 0.2).
    \end{itemize}
\end{block}
...
\begin{frame}{Phân tích Cảm xúc (Sentiment Analysis)} Để đánh giá thái độ dư luận, hệ thống sử dụng mô hình ngôn ngữ tiền huấn luyện (Pre-trained Language Model) chuyên biệt cho tiếng Việt.
    ...
    \begin{block}{Mô hình: PhoBERT (Fine-tuned)}
        \begin{itemize}
            \item \textbf{Backbone:} \texttt{vinai/phobert-base}.
    ...
    Với vector đại diện văn bản $h_{CLS}$ từ tầng cuối cùng của PhoBERT, xác suất của nhãn $k$ được tính:
