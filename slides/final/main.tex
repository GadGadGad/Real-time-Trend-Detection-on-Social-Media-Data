...
\begin{frame}{Ngăn xếp Công nghệ lõi (Core Tech Stack)}
    \begin{itemize}
        ...
        \item \textbf{Generative AI:} Google Gemini Pro / Gemma 2 (Hybrid Reasoning Strategy).
...
\begin{frame}{Cross-Encoder Reranking: Tinh chỉnh độ chính xác}
    ...
    \begin{block}{Chiến lược Reranking}enri
...
\begin{frame}{Phân tích Cảm xúc (Sentiment Analysis)} Để đánh giá thái độ dư luận, hệ thống sử dụng mô hình ngôn ngữ tiền huấn luyện (Pre-trained Language Model) chuyên biệt cho tiếng Việt.
    ...
    \begin{block}{Mô hình: ViSoBERT (Fine-tuned)}
        \begin{itemize}
            ...
            \item \textbf{Task:} Phân loại chuỗi (Sequence Classification) vào 3 nhãn: \textbf{\textcolor{forestgreen}{Positive}}, \textcolor{red}{Negative}, \textcolor{gray}{Neutral}.
        \end{itemize}
    \end{block}
    ...
    \begin{table}[]
        \resizebox{\textwidth}{!}{%
            \begin{tabular}{l c c c c c c}
                ...
                \midrule
                \textbf{vietnamese-document-embedding*} & \textbf{0.265}   & 0.239          & 0.224           & 0.162             & \textbf{0.283} & \textbf{0.419}  \\
                vietnamese-bi-encoder                   & 0.193            & 0.155          & 0.213           & 0.107             & 0.108          & 0.381           \\
                vietnamese-sbert                        & 0.188            & 0.355          & 0.172           & 0.033             & \alert{-0.005} & 0.386           \\
                bge-m3                                  & 0.160            & \alert{-0.003} & 0.269           & 0.070             & 0.192          & 0.273           \\
...
% Frame 4.2b: Phân biệt vai trò Airflow và Zookeeper
\begin{frame}{Phân biệt vai trò: Airflow vs Zookeeper}
    Hệ thống sử dụng hai cơ chế điều phối chuyên biệt để đảm bảo tính ổn định:

    \begin{columns}[t]
        \column{0.5\textwidth}
        \begin{exampleblock}{Airflow}
            \textbf{Tầng điều phối nghiệp vụ (Logical Level)}
            \begin{itemize}
                \item Quản lý và lập lịch các tác vụ (\textit{Task Scheduling}).
                \item Điều phối luồng dữ liệu thông qua định nghĩa DAG.
                \item Giám sát (\textit{Monitoring}) và xử lý lỗi tự động (\textit{Retry mechanism}).
            \end{itemize}
        \end{exampleblock}

        \column{0.5\textwidth}
        \begin{block}{Zookeeper}
            \textbf{Tầng điều phối hạ tầng (Infrastructure Level)}
            \begin{itemize}
                \item Quản lý cấu hình cụm **Kafka** (\textit{Cluster Configuration}).
                \item Thực hiện bầu chọn Leader (\textit{Leader Election}) cho các partitions.
                \item Đồng bộ trạng thái và phát hiện lỗi node (\textit{Failure Detection}).
            \end{itemize}
        \end{block}
    \end{columns}
    ...
    \vspace{0.3cm}
    \centering
    \textit{$\Rightarrow$ Airflow quản trị **Luồng nghiệp vụ**, Zookeeper quản trị **Hạ tầng phân tán**.}
\end{frame}
    ...
    \begin{itemize}
        \item \textbf{1. Sentiment Analysis (4,630 samples):}
              \begin{itemize}
                  ...
                  \item Pseudo-labeling bằng GPT-4o, sau đó **Human Review** các trường hợp có độ tin cậy thấp.
              \end{itemize}
...
\begin{frame}{Chi tiết Huấn luyện \& Fine-tuning}
    ...
    \begin{columns}
        \column{0.6\textwidth}
        \small
        \textbf{Chiến lược Reranker:**} Tuy số lượng ít (877 cặp) nhưng đây là dữ liệu **"Gold Standard"** chất lượng cao, tập trung vào các câu query khó (Hard Negatives) để model học được ranh giới quyết định tốt nhất.

        \column{0.4\textwidth}
        \begin{center}
            \includegraphics[width=0.9\textwidth]{figures/llm_finetuned(1).png}
        \end{center}
    \end{columns}
\end{frame}
...
\begin{frame}{Kết quả Phân loại \& Tinh chỉnh (LLM Refinement)}
    ...
    \begin{columns}[t]
        ...
        \begin{block}{Intelligence (5W1H)}
            ...
            \begin{itemize}
                ...
                \item \textbf{Advice (State):} Nhà nước nên kích hoạt phương án ứng phó thiên tai cấp 3, huy động lực lượng sơ tán dân vùng trũng.
                \item \textbf{Advice (Business):} Các doanh nghiệp cần rà soát an toàn kho bãi, gia cố tài sản và kích hoạt chế độ làm việc từ xa (WFH).
            \end{itemize}
        \end{block}
    \end{columns}
    ...
    \begin{exampleblock}{Tóm tắt ngữ nghĩa (LLM Summary)}
        \scriptsize
        \textit{"Cụm này nói về bão số 3 (Yagi) đã đổ bộ trực tiếp vào các tỉnh Quảng Ninh và Hải Phòng với sức gió mạnh cấp 12, gây mưa lớn và ngập lụt diện rộng. Hàng loạt cây xanh bị gãy đổ và nhiều khu vực bị mất điện. Chính quyền đã sơ tán khẩn cấp người dân đến nơi an toàn."}
    \end{exampleblock}
\end{frame}
    ...
    \begin{enumerate}
        ...
        \item \textbf{Real-time Processing:}
              \begin{itemize}
                  ...
                  \item Tự động kích hoạt thuật toán đề xuất và gọi LLM để gán nhãn xu hướng ngay trên luồng.
              \end{itemize}
        ...
        \item \textbf{Intelligence Analysis:} Sử dụng tính năng \textit{"Xem chi tiết"} để kiểm tra:
              \begin{itemize}
                  \item Tóm tắt nội dung, Lời khuyên hành động (State/Business) và Giải thích từ AI.
              \end{itemize}
        \item \textbf{System Audit:} Theo dõi hiệu suất xử lý Micro-batch và tỉ lệ AI Analysis (Deep vs Fast Path).
    \end{enumerate}
\end{frame}
