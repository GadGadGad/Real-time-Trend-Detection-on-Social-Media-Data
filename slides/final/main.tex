...
% Frame 3.1: Kiến trúc Hybrid & Model
\begin{frame}{Kiến trúc Mô hình đề xuất (Proposed Architecture)}
    Hệ thống sử dụng phương pháp tiếp cận \textbf{Hybrid (Lai ghép)}, kết hợp sức mạnh thống kê (Statistical NLP) và Generative AI:

    \begin{enumerate}
        \item \textbf{Tiền xử lý (Denoising):} Lọc nhiễu đặc thù báo chí và tin rác.
        \item \textbf{Vector hóa (Embedding):} Chuyển đổi văn bản sang không gian ngữ nghĩa đa ngôn ngữ.
        \item \textbf{Phân cụm lai (SAHC):} Thuật toán ưu tiên nguồn tin cậy (News) để dẫn dắt nguồn nhiễu (Social).
        \item \textbf{Hậu xử lý (LLM Refinement):} Sinh tri thức (Knowledge Generation) từ cụm thô.
    \end{enumerate}

    \begin{block}{Công nghệ lõi}
        \begin{itemize}
            \item \textbf{Embedding Model:} \texttt{dangvantuan/document-embedding}.
            \item \textbf{Clustering:} Cơ chế SAHC tùy biến trên nền tảng HDBSCAN.
            \item \textbf{LLM Backend:} Google Gemini (gemma-3-27b-it) cho tác vụ 5W1H.
        \end{itemize}
    \end{block}
\end{frame}
...
\begin{frame}{Thuật toán SAHC: Mô hình Toán học (2/2)}
    ...
    \begin{block}{Bước 2: Social Attachment Decision (Quyết định gắn kết)}
        ...
        \vspace{0.2cm}
        \textit{\small(Với ngưỡng thực nghiệm $\tau = 0.65$ giúp lọc bỏ nhiễu tin rác)}
    \end{block}
\end{frame}
...
\begin{frame}{Thuật toán SAHC: Ví dụ Minh họa (Working Example)}
    ...
    \begin{exampleblock}{Scenario: Sự kiện Bão Yagi}
        ...
        \textbf{Kết luận:} Vì $0.72 > 0.65$ (Threshold) $\rightarrow$ Bài viết được gộp vào cụm $C_1$ (News Leading).
    \end{exampleblock}
\end{frame}
...
\begin{frame}{Phân loại Taxonomy: Cơ chế Hybrid Scoring}
    Hệ thống phân loại sự kiện vào 7 nhóm (T1-T7) dựa trên cơ chế chấm điểm trọng số từ khóa (Keyword Weighted Scoring).

    \begin{block}{Công thức tính điểm chủ đề (Topic Scoring)}
        Điểm số của văn bản $D$ đối với chủ đề $T_i$ được tính như sau:
        \begin{equation}
            \text{Score}(D, T_i) = \sum_{w \in W_{D}} \mathbb{I}(w \in K_{T_i}) \times \text{weight}(w)
        \end{equation}
        \textit{Trong đó:}
        \begin{itemize}
            \item $K_{T_i}$: Bộ từ khóa định nghĩa cho chủ đề $T_i$ (Ví dụ T1: [lũ lụt, sập, chết người]).
            \item $\text{weight}(w)$: Trọng số quan trọng (Critical keywords = 2.0, Normal = 1.0).
        \end{itemize}
    \end{block}

    \textbf{Quy tắc quyết định:}
    Chủ đề $T_{final} = \arg\max_{T_i} (\text{Score}(D, T_i))$. Nếu $\max < \text{min\_threshold}$, gán nhãn "Routine" (T7).
\end{frame}
...
\begin{frame}{Kiến trúc xử lý thời gian thực (Streaming Architecture)}
    ...
    \begin{itemize}
        \item \textbf{Data Producers:}
        \begin{itemize}
            \item Các Crawlers (News) và API Collectors (Social) hoạt động đa luồng.
            \item Dữ liệu được chuẩn hóa JSON và đẩy vào \textbf{Kafka Topic} ('raw\_data').
        \end{itemize}
        
        \item \textbf{Streaming Engine (Spark):}
        \begin{itemize}
            \item Subscribe vào Kafka topic.
            \item Xử lý theo cơ chế \textbf{Micro-batch} (Chu kỳ 15 giây).
...
\begin{frame}{Xử lý phân tán trên Spark (Distributed Processing)}
    Quy trình xử lý một Micro-batch trong \texttt{trend\_detection\_job.py}:

    \begin{block}{1. Distributed Embedding (UDF)}
        \begin{itemize}
            \item Sử dụng \textbf{Pandas UDF} (User Defined Function) để vector hóa văn bản song song trên các Worker Nodes.
            \item Kỹ thuật \textbf{Model Broadcasting}: Tải model NLP một lần trên mỗi Executor để tối ưu hóa bộ nhớ (thay vì tải lại cho từng bản ghi).
        \end{itemize}
    \end{block}
...
\begin{frame}{So sánh Hiệu năng các mô hình (Model Comparison)}
    ...
    \begin{table}[]
        ...
        \begin{tabular}{l c c c}
            ...
            \textbf{Entropy} ($\downarrow$)    & 4.85          & 6.81 & 5.02 \\
            \bottomrule
        \end{tabular}%
        }
...
\begin{frame}{Kịch bản Demo}
    ...
    \begin{enumerate}
        \item \textbf{Stream Simulation:} Kích hoạt module \texttt{Replay Producer} để đẩy dữ liệu giả lập vào Kafka, mô phỏng tốc độ tin tức thực tế (Velocity).
        \item \textbf{Real-time Processing:}
              \begin{itemize}
                  \item Engine Spark Streaming tiếp nhận dữ liệu theo Micro-batch (chu kỳ 15s).
...
% Frame 6.2: Giao diện kết quả (Chỉ chứa ảnh)
\begin{frame}{Giao diện Dashboard }
    \begin{figure}[h]
        \centering
        % [LƯU Ý] Bạn cần thay 'images/dashboard_ui.png' bằng đường dẫn file ảnh chụp màn hình thực tế của bạn
        % Tỷ lệ width=0.95\textwidth giúp ảnh to rõ, tràn slide
        % \includegraphics[width=0.95\textwidth]{images/dashboard_ui.png}
        
        % Nếu chưa có ảnh, dùng lệnh này để tạo khung giữ chỗ (Placeholder box)
        \framebox(300,180){[PLACEHOLDER: ẢNH CHỤP MÀN HÌNH DASHBOARD]}
