...
\begin{frame}{Kế hoạch}
  ...
  \begin{table}
    ...
    {@{\hspace{6pt}}p{0.1\textwidth}
     p{0.58\textwidth}
     p{0.22\textwidth}@{\hspace{6pt}}}
      \toprule
      \textbf{Task} & \textbf{Mô tả} & \textbf{Trạng thái} \\
      \midrule

      Task 1 & \raggedright Thu thập dữ liệu đa nguồn (Crawler / Apify / News) & \textbf{Đã hoàn thành} \\

      Task 2 & \raggedright Chuẩn hoá schema + làm sạch dữ liệu (Unicode, nội dung mẫu lặp) & Đang làm \\

      Task 3 & \raggedright Semantic matching (embedding) + tăng cường ngữ cảnh (NER / alias) & Đang thử nghiệm \\

      Task 4 & \raggedright Tối ưu phân cụm (UMAP + HDBSCAN) + đánh giá chất lượng cụm & Đang tìm giải pháp tối ưu hơn \\

      Task 5 & \raggedright Trend scoring để xác định xu hướng & Đang thử nghiệm \\

      Task 6 & \raggedright Đặt tên trend và tóm tắt thông tin cốt lõi cho mỗi cụm & TBD \\

      Task 7 & \raggedright Phân loại cảm xúc, tâm lý cộng đồng theo từng trend & TBD \\

      Task 8 & \raggedright Dashboard/báo cáo + tích hợp pipeline định kỳ/streaming & TBD \\
      \bottomrule
    \end{tabular}
...
%   \begin{enumerate}
%     \item \textbf{Social Controversy:} Tranh cãi xã hội, sự kiện nóng, bê bối (Ưu tiên cao cho CP).
...
%------------------------------------------------------------
\section{Dữ liệu \& Hướng thử nghiệm}
...
%   \justifying
%   Hệ thống chia làm 2 pha chính (phục vụ phát hiện xu hướng và phân tích phản ứng cộng đồng theo thời gian thực):
%   \begin{columns}
%     \column{0.5\textwidth}
...
%       \item Vector hoá (embedding) và \textbf{phân cụm}.
%       \item Tạo luật/gán nhãn bán giám sát để phát hiện và phân loại trend.
%     \end{itemize}
...
%       \item \textbf{Phân loại cảm xúc / tâm lý cộng đồng theo từng trend}.
...
%     \textbf{Phát hiện xu hướng là bước nền tảng}; phân tích cảm xúc/tâm lý được thực hiện sau khi trend đã được xác định rõ.
%   \end{block}
% \end{frame}
...
\begin{frame}{Pipeline NLP phát hiện xu hướng (đang phát triển)}
  ...
  \begin{itemize}
    % \item \textbf{Tiền xử lý:} chuẩn hoá Unicode, loại nội dung mẫu lặp, làm sạch ký tự/emoji.
    ...
    \item \textbf{Tăng cường ngữ cảnh (2 hướng thử nghiệm):}
          ...
          \end{itemize}
    \item \textbf{Ghép \& gom topic:} cosine similarity (matching) + UMAP (giảm chiều) + HDBSCAN (phân cụm).
  \end{itemize}
\end{frame}

\begin{frame}{So sánh trực quan: phân cụm với NER}
  \justifying
  ...
  \begin{figure}
    \centering
    \includegraphics[width=0.55\textwidth]{assets/images/cluster.png}
...
\begin{frame}{So sánh trực quan: phân cụm với Alias từ Google Trends}
  \justifying

  \begin{minipage}{\linewidth}
    \begin{block}{Thiết lập}
      {\footnotesize
        Embedding tiếng Việt (PhoBERT / bi-encoder) + Cosine Matching (Alias normalization từ Google Trends)
        $\rightarrow$ UMAP + HDBSCAN.
      }
    \end{block}
  \end{minipage}
...
\begin{frame}{Trend Scoring \& Phân loại trend (ý tưởng)}
  \justifying
...
\begin{frame}{Kết quả thử nghiệm bước đầu}
  ...
  \begin{columns}
    ...
    \begin{block}{Những điểm đã đạt được}
      \begin{itemize}
        \item Đã chạy được pipeline: \textbf{chuẩn hoá} $\rightarrow$ \textbf{embedding} $\rightarrow$ \textbf{matching} $\rightarrow$ \textbf{phân cụm}.
    ...
    \begin{alertblock}{Vấn đề còn tồn tại}
      \begin{itemize}
        ...
        \item \textbf{Nếu phụ thuộc quá vào từ khóa google trend, sẽ kém hiệu quả với các trend mới hoàn toàn.}
      \end{itemize}
    \end{alertblock}
  \end{columns}
...
\begin{frame}{Khó khăn hiện tại}
  ...
  \begin{enumerate}
    \item \textbf{Vấn đề Phân cụm (phân cụm):}
          \begin{itemize}
    ...
    \item \textbf{Đồng bộ thời gian đa nguồn:}
          \begin{itemize}
            \item Social nhanh, News có độ trễ, Search phản ánh ý định theo ngữ cảnh cần có cơ chế đồng bộ công bằng.
          \end{itemize}
  \end{enumerate}
\end{frame}
