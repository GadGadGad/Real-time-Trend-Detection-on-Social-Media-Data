...
{\textbf{Thực hiện:} \break Tăng Nhất\inst{1} \and Lê Minh Nhựt\inst{1} \break \textbf{GVHD}: TS. Đỗ Trọng Hợp \break Thầy Nguyễn Ngọc Quí}
...
\begin{frame}[fragile]{Nguồn dữ liệu 1: Mạng xã hội (Social Media)}
  \justifying
  \begin{itemize}
    \item \textbf{Nguồn:} Các fanpage lớn / trang tin trên Facebook (ví dụ: Theanh28, Thông tin Chính Phủ, ...).
    \item \textbf{Định dạng:} JSON (nội dung, thời gian đăng, chỉ số tương tác).
    \item \textbf{Tín hiệu trend:} likes/comments/shares (có thể chuẩn hoá theo thời gian).
    \item \textbf{Trạng thái:} \textbf{Đã hoàn thành crawl} và đưa về schema thống nhất.
  \end{itemize}
...
\begin{frame}{So sánh trực quan: Clustering với NER}
  ...
  \begin{figure}
    \centering
    \includegraphics[width=0.5\textwidth]{assets/images/ner.png}
    \caption{Phân bố cụm khi tăng cường ngữ cảnh bằng Named Entity Recognition}
  \end{figure}
...
\begin{frame}{So sánh trực quan: Clustering với Alias từ Google Trends}
  ...
  \begin{figure}
    \centering
    \includegraphics[width=0.5\textwidth]{assets/images/ggtrend.png}
    \caption{Phân bố cụm khi mở rộng biến thể từ khoá từ Google Trends}
  \end{figure}
...
\begin{frame}{Kế hoạch tiếp theo (Timeline)}
  ...
  \begin{table}
    ...
    \begin{tabular}{p{0.12\textwidth} p{0.62\textwidth} p{0.20\textwidth}}
      ...
      Task 3 & Semantic matching (embedding) + alias/NER enrichment & Đang làm \\
      Task 4 & Tối ưu Clustering (UMAP + HDBSCAN) + đánh giá chất lượng cụm & TBD \\
