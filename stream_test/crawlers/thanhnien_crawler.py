...
class ThanhNienCrawler:
    ...
    def discover_articles(self, category_slug, pages, progress_context=None, task_id=None):
        ...
        for p in range(1, pages + 1):
            url = f"{BASE}/{category_slug}.htm" if p == 1 else f"{BASE}/{category_slug}/trang-{p}.htm"
            html = self.safe_get(url)
            if not html: continue

            soup = BeautifulSoup(html, "lxml")
            for item in soup.select(".box-category-item, .story"):
                link_el = item.select_one("a.box-category-link-title, a.story__title")
    ...
    def fetch_article(self, url, short_description, category_source):
        ...
        content_el = soup.select_one(".detail-content")
