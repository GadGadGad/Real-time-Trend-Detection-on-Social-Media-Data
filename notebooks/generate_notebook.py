 ...
 "cells": [
  ...
  },
  {
   ...
   "source": [
    ...
    "MIN_CLUSTER_SIZE = 3 # HDBSCAN min cluster size (Lower = more micro-clusters)\n",
    "EMBEDDING_METHOD = \"sentence-transformer\" # Options: 'sentence-transformer', 'tfidf', 'bow'\n",
    "LABELING_METHOD = \"semantic\" # Options: 'semantic' (KeyBERT-style), 'tfidf' (Frequency)"
   ]
  },
  {
   "cell_type": "markdown",
  ...
  },
  {
   ...
   "source": [
    ...
    "    embedding_method=EMBEDDING_METHOD, # Dynamic embedding method\n",
    "    labeling_method=LABELING_METHOD    # Dynamic labeling method\n",
    ")\n",
    "df_hyb = pd.DataFrame(matches_hybrid)\n",
    "print(\"Hybrid Topics Found:\", df_hyb['final_topic'].nunique())\n",
    "\n",
    "    # improved display with new metrics\n",
    "    cols = ['final_topic', 'topic_type', 'trend_score', 'sentiment', 'post_content']\n",
